...
def expandir_dims(img):
    image = cv2.resize(img, (28,28)).astype("float32")#define o tipo da imagem pra float32
...
def processarImg(imagem):
    img_processada = processar(imagem)
    return img_processada


# Processando as imagens importadas pela função acima
def processar_imagens_importadas(imgs_importadas):
    ...
    for lista_digito_img in imgs_importadas:
        ...
        for imagem in lista_digito_img:
            img_processed = processar(imagem)
...
def classificar():
    ...
    imgs_importadas = importar_dataset(path)
    lista_imgs = processar_imagens_importadas(imgs_importadas)

    print("começou a classificação")

    interpreter = tflite.Interpreter("model45.tflite")  # importa o modelo model.tflite
    ...
    for digito, lista_digito in enumerate(lista_imgs):
        ...
        for img_processada in lista_digito:
            img_expanded = expandir_dims(img_processada)
    ...
    print("Resultados do modelo com 45 epocas")    
    ...
    for digito, acuracia in enumerate(acuracia_por_digito):
        ...
        valores.append(
            f"{(acuracia_por_digito[digito] / qtd_imgs_por_digito[digito]) * 100:.2f}%".rjust(
                tam[3]
            )
        )
        linha = "   ".join(valores)
